...
function renderLive(raw){
  ...
  const commentary=prepared.map((d,i)=>{
    ...
    const newcomers=[d.batsman,d.non_striker].filter(n=>n&&!seenBatters.has(n));
    newcomers.forEach(n=>seenBatters.add(n));
    return commentaryHTML(md,newcomers,m.format,d.display_ball);
  });
...
​
  const teamSummary=team=>{
    ...
    return innings.map(n=>{
      const part=prepared.filter(d=>Number(d.innings||1)===n);
      const x=calcMatch(part,m,n);
...
​
  app.innerHTML=`<div class="score-page">
    ...
    <section class="espn-match-header">
      <div class="match-kicker"><span class="match-status ${resultText?"is-result":"is-live"}">${esc(statusLabel)}</span><span>${esc(m.format||"Match")} · ${esc(m.match_id||"")}${testDayLabel}</span></div>
      <h1>${esc(a)} vs ${esc(b)}</h1>
      <p class="match-subline">${esc(m.venue||"Noida Intercollege Cricket Ground")}</p>
      <div class="team-scoreboard">
        <div class="team-score-row ${state.team===a?"is-batting":""}">
          <div class="team-identity">${logo(a,"score-team-logo")}<div><b>${esc(a)}</b><small>${state.team===a&&!resultText?"Batting":""}</small></div></div>
          <div class="team-score-value">${teamSummary(a)}</div>
        </div>
        <div class="team-score-row ${state.team===b?"is-batting":""}">
          <div class="team-identity">${logo(b,"score-team-logo")}<div><b>${esc(b)}</b><small>${state.team===b&&!resultText?"Batting":""}</small></div></div>
          <div class="team-score-value">${teamSummary(b)}</div>
        </div>
      </div>
    ...
    <div class="score-layout">
      <main class="score-main">
        <section class="live-strip">
          <div><span class="live-dot"></span><b>${esc(statusLabel)}</b><small>Innings ${currentInnings}</small></div>
          <div class="live-total"><strong>${state.runs}/${state.wickets}</strong><span>${state.overs} ov · CRR ${state.crr}</span></div>
          <div class="live-batters"><span><b>${esc(state.striker?.name||"—")}*</b> ${state.striker?state.striker.runs+" ("+state.striker.balls+")":""}</span><span>${esc(state.nonStriker?.name||"—")} ${state.nonStriker?state.nonStriker.runs+" ("+state.nonStriker.balls+")":""}</span></div>
        </section>
...
​
      <aside class="score-sidebar">
        <section class="espn-panel"><div class="panel-heading"><h2>Match details</h2></div><dl class="match-details">
          ...
          <div><dt>Toss</dt><dd>${esc(tossSummary(m))}</dd></div>
          <div><dt>Umpires</dt><dd>${esc([m.umpire_1,m.umpire_2].filter(Boolean).join(" · ")||"—")}</dd></div>
          <div><dt>Partnership</dt><dd>${state.partnership.runs} runs · ${state.partnership.balls} balls</dd></div>
          <div><dt>Current bowler</dt><dd>${esc(state.bowler.name||"—")} · ${state.bowler.overs} · ${state.bowler.runs}/${state.bowler.wickets}</dd></div>
        </dl></section>
...
function bowlerRunsForDelivery(d){
  ...
  if(parts){
    const nonBowler=Number(parts.byes||0)+Number(parts.legbyes||parts.leg_byes||0)+Number(parts.penalty||parts.penalty_runs||0);
    return batterRuns+Math.max(0,extras-nonBowler);
  }
  return /^(bye|byes|leg-bye|leg-byes|penalty|penalty-runs)$/.test(normalizedExtraType(d))
    ? batterRuns
    : batterRuns+extras;
}
function wicketType(d){
  return String(d?.wicket_type||d?.dismissal_type||"")
    .trim().toLowerCase().replace(/[\s_-]+/g,"");
}
function wicketAllowedOnFreeHit(d){
  return ["runout","obstructingthefield","hittheballtwice"].includes(wicketType(d));
}
function wicketCountsForBowler(d){
  return !["runout","retiredhurt","retiredout","obstructingthefield","timedout","hittheballtwice"].includes(wicketType(d));
}
function isRunOut(d){return wicketType(d)==="runout"}
function resolveDismissedPlayer(d,striker,nonStriker){
  const explicit=String(d?.dismissed_player||d?.run_out_batter||"").trim();
  if(explicit)return explicit;
  if(!isRunOut(d))return striker;
  const end=String(d?.dismissed_end||d?.run_out_end||d?.wicket_end||"")
    .trim().toLowerCase().replace(/[\s_-]+/g,"");
  if(["striker","batter","batting"].includes(end))return striker;
  if(["nonstriker","nonbatter"].includes(end))return nonStriker;
  return "";
}
function validateDeliveryFeed(deliveries){
  deliveries.forEach((d,index)=>{
    const label=`Delivery ${index+1}`;
    if(Number(d?.extra_runs||0)>0&&!normalizedExtraType(d)&&!(d?.extras&&typeof d.extras==="object")){
      throw new Error(`${label}: extra_runs requires extra_type or an extras breakdown.`);
    }
    if(Number(d?.wicket||0)>0&&isRunOut(d)&&
       !String(d?.dismissed_player||d?.run_out_batter||d?.dismissed_end||d?.run_out_end||d?.wicket_end||"").trim()){
      throw new Error(`${label}: a run-out requires dismissed_player or dismissed_end.`);
    }
  });
}
​
...
function deliveryIsFreeHit(freeHitPending){
  return Boolean(freeHitPending);
}
...
​
  ...
  const feedOrder=[];
  if(suppliedXI.length<2){
    for(const d of current){
      for(const raw of [d.batsman,d.striker,d.non_striker]){
        const name=String(raw||"").trim();
        if(name&&!feedOrder.includes(name))feedOrder.push(name);
      }
    }
  }
  const battingOrder=[...(suppliedXI.length>=2?suppliedXI:feedOrder)];
  let striker=battingOrder[0]||"";
...
​
  ...
  const takeNextBatter=()=>{
    while(nextBatterIndex<battingOrder.length){
      const candidate=battingOrder[nextBatterIndex++];
      if(candidate&&candidate!==striker&&candidate!==non&&!bat[candidate]?.out)return candidate;
    }
    return "";
  };
...
​
    ...
    const extras=Number(d.extra_runs??0);
    const total=Number(d.total_runs??(batterRuns+extras));
...
​
    ...
    if(bat[bats]){
      bat[bats].runs+=batterRuns;
      if(batterRuns===4)bat[bats].fours++;
...
​
    ...
    if(bowler){
      ...
      bowl[bowler].runs+=bowlerRunsForDelivery(d);
...
​
      ...
      let nextNon=total%2===1?bats:nonStrikerAtStart;
      const incoming=wickets<10?takeNextBatter():"";
      if(incoming){
        ensureBatter(incoming);
        if(dismissed===nextStriker)nextStriker=incoming;
        else if(dismissed===nextNon)nextNon=incoming;
      }
      if(overEnds)[nextStriker,nextNon]=[nextNon,nextStriker];
      striker=nextStriker;non=nextNon;
...
​
  ...
  const reachedOverLimit=maxOvers!==null&&legal>=maxOvers*6;
  const inningsEndFlag=current.length>0&&Number(current[current.length-1]?.innings_end||0)===1;
  const allOut=wickets>=10;
  const complete=Boolean(reachedOverLimit||inningsEndFlag||allOut);
  return {team_a:st(m.team_a),team_b:st(m.team_b),team:battingTeam,innings:inn,
    runs,wickets,overs:`${Math.floor(legal/6)}.${legal%6}`,legalBalls:legal,
    crr:legal?(runs/(legal/6)).toFixed(2):"0.00",striker:s,nonStriker:n,
...
function ballChip(d){
 ...
 const val=d.wicket
   ?"W"
   :d.extra_type
     ?String(d.extra_type).toUpperCase()+(d.extra_runs?` ${d.extra_runs}`:"")
     :String(d.total_runs??batterRuns);
 let cls=d.wicket?"wicket":batterRuns===6?"six":batterRuns===4?"four":d.extra_type?"extra":"";
 return `<span class="ball ${cls}" title="${esc(d.commentary||"")}">${esc(val)}</span>`;
}
function commentaryHTML(d,newcomers=[],format="",displayBall=""){
  const r=Number(d.runs_off_bat??d.runs??0);
...
function battingCard(state,m){
  ...
  const rows=names.map(name=>{
    ...
    if(!x)return `<tr class="yet-row"><td><span class="player-name">${esc(name)}</span><small>yet to bat</small></td><td colspan="6">—</td></tr>`;
    const isStriker=name===state.striker?.name;
    const status=x.out?"out":(x.seen||x.balls>0?"not out":"yet to bat");
    return `<tr><td><span class="player-name">${isStriker?"● ":""}${esc(name)}${isStriker&&!x.out?" *":""}</span><small>${status}</small></td><td class="num strong">${x.runs}</td><td class="num">${x.balls}</td><td class="num">—</td><td class="num">${x.fours}</td><td class="num">${x.sixes}</td><td class="num">${x.balls?(x.runs/x.balls*100).toFixed(2):"0.00"}</td></tr>`;
  }).join("");
  return `<div class="score-table-scroll"><table class="espn-score-table"><thead><tr><th>Batting</th><th>R</th><th>B</th><th>M</th><th>4s</th><th>6s</th><th>SR</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
function bowlingCard(state){
  const rows=Object.values(state.bowl).map(x=>`<tr><td><span class="player-name">${esc(x.name)}</span></td><td class="num">${Math.floor(x.legal/6)}.${x.legal%6}</td><td class="num">—</td><td class="num">${x.runs}</td><td class="num strong">${x.wickets}</td><td class="num">${x.legal?(x.runs/(x.legal/6)).toFixed(2):"0.00"}</td><td class="num">—</td></tr>`).join("");
  return `<div class="score-table-scroll"><table class="espn-score-table"><thead><tr><th>Bowling</th><th>O</th><th>M</th><th>R</th><th>W</th><th>Econ</th><th>0s</th></tr></thead><tbody>${rows||`<tr><td colspan="7" class="empty-cell">No bowling figures yet.</td></tr>`}</tbody></table></div>`;
}
function allInningsCards(ds,m){
  ...
  return Array.from({length:count},(_,index)=>{
    ...
    if(!inningsData.length)return `<section class="espn-panel innings-panel is-pending"><div class="innings-heading"><div><span>Innings ${innings}</span><h2>${esc(battingTeam)}</h2></div><b>Yet to bat</b></div></section>`;
    return `<section class="espn-panel innings-panel">
      <div class="innings-heading"><div><span>Innings ${innings}</span><h2>${esc(battingTeam)}</h2></div><div class="innings-total"><b>${state.runs}/${state.wickets}</b><span>${state.overs} overs</span></div></div>
      ${battingCard(state,m)}
      <div class="innings-summary-row"><b>Total</b><span>${state.runs}/${state.wickets} <small>(${state.overs} Ov, RR: ${state.crr})</small></span></div>
      <div class="bowling-title">${esc(bowlingTeam)} bowling</div>
      ${bowlingCard(state)}
    </section>`;
  }).join("");
}
function playingXI(m,a,b){
  const ix=m.playing_xi||{},squads=DATA.squads||{};
  ...
  function box(team){
    ...
    return `<div class="lineup-card"><h3>${esc(team)}</h3><ol>${names.slice(0,11).map(n=>`<li><span>${esc(n)}</span><small>${esc(roleFor(team,n))}</small></li>`).join("")}</ol></div>`;
  }
...
function calculatedTeamRecords(){
 ...
 for(const m of (DATA.live_matches||[])){
   if(String(m.status||"").toLowerCase()!=="completed"||m.player_records_enabled!==true)continue;
